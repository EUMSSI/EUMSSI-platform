 ...
 * 
 */
@Path("/queue/processed")
public class QueueProcessed {
	...
	public QueueProcessed() throws UnknownHostException, EumssiException {
		this.queryManager = QueryManager.getInstance();
	}

	/**
	 * Returns list of already processed content items
	 * 
	...
	public Response queueProcessedServiceGET(
			@QueryParam("queueId") String queueId,
			@QueryParam("maxItems") @DefaultValue("1000") Integer maxItems,
			@QueryParam("markItems") @DefaultValue("False") Boolean markItems,
		...
		try {
			...
			// get item list
			List<String> itemList = queryManager.getQueueProcessed(queueId, maxItems, markItems, filters);
